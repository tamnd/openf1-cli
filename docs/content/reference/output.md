...
describes what users get. Pick a format with `-o`, or let openf1 choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
openf1 <command> -o table     # a rounded, color-aware grid for reading
openf1 <command> -o markdown  # a GitHub pipe table to paste into docs (alias: md)
openf1 <command> -o list      # one record per section, easy on the eyes
openf1 <command> -o jsonl     # one JSON object per line, for piping
openf1 <command> -o json      # a single JSON array
openf1 <command> -o csv       # spreadsheet friendly
openf1 <command> -o tsv       # tab-separated
openf1 <command> -o url       # just the URL column
openf1 <command> -o raw       # the underlying bytes, unformatted
```
...
```bash
openf1 <command> --fields id,title,url
```
...
```bash
openf1 <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
openf1 <command>            # a table, because this is a terminal
openf1 <command> | wc -l    # JSONL, because this is a pipe
```
