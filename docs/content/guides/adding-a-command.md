---
title: "Add a command"
description: "Model a real openf1 record and expose it as a command, a route, and a tool at once."
weight: 10
---

The scaffold ships one example type, `page`. Real work means modelling the
records openf1 actually serves. You do that in two files, and every surface
updates itself.

## 1. Model the record

In `openf1/openf1.go`, add a struct for the thing you are fetching
...
type Item struct {
    ...
    Owner string `json:"owner" kit:"link,kind=openf1/user"` // an edge to another record
}
...
- `kit:"link,kind=<scheme>/<type>"` marks an outbound edge. It can point at
  another openf1 type or at another site entirely, which is what lets a
  host walk the graph across tools.

## 2. Declare the operation

In `openf1/domain.go`, add an input struct and a handler, then register
it in `Register`:
...
```bash
openf1 item <id>                 # the command
curl 'localhost:7777/v1/item/<id>'      # the route, under serve
ant get openf1://item/<id>       # the URI dereference, via a host
```
