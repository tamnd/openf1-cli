...
// HTTP behaviour is covered in openf1_test.go.
...
func TestHostWiring(t *testing.T) {
	...
	if want := "openf1://page/wiki/Go"; u.String() != want {
	...
	if !h.Searchable("openf1") {
		t.Error("Searchable = false, want true (the domain registers a search op)")
	}

	got, err := h.ResolveOn("openf1", "about")
	if err != nil || got.String() != "openf1://page/about" {
		t.Errorf("ResolveOn = (%q, %v), want openf1://page/about", got.String(), err)
	}
}
